...
class copyDirectories:
    ...
    def use_shutil(self, start_index :int = 0):
        ...
        for i in range(start_index, pathListLen):
            ...
            print(f'[{i:>5d}/{pathListLen}]-[{path_type}]: '
                f'{src_path_preview[:48]:<48s} {dst_path_preview[:64]}')
            ...
            try:
                if path_type == self.IS_DIR:
                    ...
                    try:
                        ...
                        message = f'File Exists Error: \'{src_path_preview}\''
                        print(message)
                        fwrite('shutil_copy.log', f'{message}\n', 'a+')
                else:
                    message = f'Strange file type: \'{src_path_preview}\''
                    print(message)
                    fwrite('shutil_copy.log', f'{message}\n', 'a+')
            except (PermissionError, shutil.Error) as message:
                print(message)
                fwrite('shutil_copy.log', f'{message}\n', 'a+')

def shutil_copy_test(src :str, dst :str):
    shutil.copy(src, dst)
    return sha256(src), sha256(dst)

def zipfile_test():
    zipFile = zipfile.ZipFile('test.zip', 'w')
    zipFile.setpassword(b'12345678')
    zipFile.write('135315324', '5135135')
    zipFile.close()
